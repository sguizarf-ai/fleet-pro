        ...
        <div className="mbody" style={{flex:1,overflowY:"auto",minHeight:0}}>
          ...
          <div className="fg">
            ...
            <div className="field" style={{position:"relative"}}>
              <label>Origen *</label>
              <input value={f.origen} onChange={ch("origen")} placeholder="Ej: Caterpillar Santa Catarina / Zona Industrial" list="origenes-list"/>
            ...
            <div className="field s2"><label>Unidad *</label><select value={f.unidadId} onChange={e=>{const u=(units||[]).find(u2=>u2.id===e.target.value);const drv=u&&u.operador?(drivers||[]).find(d=>d.id===u.operador||d.nombre===u.operador):null;setF(p=>({...p,unidadId:e.target.value,operadorViaje:drv?.nombre||u?.operador||""}));}} style={{background:"var(--bg0)",color:"var(--text)",border:"1px solid var(--border)",borderRadius:8,padding:"9px 12px",width:"100%"}}><option value="">— Seleccionar unidad —</option>{units.map(u=><option key={u.id} value={u.id}>{u.num} — {u.placas}{u.tipo?" ("+u.tipo+")":""}</option>)}</select></div>
            <div className="field">
            ...
            <div className="field s2">
              <label>Operador del Viaje</label>
              <select value={f.operadorViaje||""} onChange={ch("operadorViaje")} style={{background:"var(--bg0)",color:"var(--text)",border:"1px solid var(--border)",borderRadius:8,padding:"9px 12px",width:"100%"}}>
                <option value="">— Operador de la unidad (por defecto) —</option>
                {(drivers||[]).filter(d=>d.activo!==false).map(d=><option key={d.id} value={d.nombre}>{d.nombre}{d.licencia?" · Lic. "+d.licencia:""}</option>)}
              </select>
            </div>
          ...
          {(Number(f.costoOfrecido) > 0 || Number(f.combustibleViaje) > 0 || Number(f.comisionViaje) > 0 || Number(f.casetas) > 0 || Number(f.gastosExtras) > 0 || Number(f.costoEstadias) > 0 || Number(f.viaticos) > 0) && <div style={{marginTop:6,marginBottom:10,padding:"12px 16px",background:utilidadPropio>=0?"#e8f8ee":"#fdecea",borderRadius:10,border:`1px solid ${utilidadPropio>=0?"var(--green)":"var(--red)"}`}}>
            ...
            <div className="field s2">
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap",padding:"8px",borderRadius:8,border:`2px solid ${(f.tipoComprobante===undefined||f.tipoComprobante===null||f.tipoComprobante==="")?"var(--orange)":"var(--border)"}`,background:"var(--bg0)"}}>
                {[["ninguno","— Sin comprobante —"],["factura","🧾 Factura (con IVA)"],["remision","📋 Remisión (sin IVA)"]].map(([v,lbl])=>(
              ...
              {(f.tipoComprobante===undefined||f.tipoComprobante===null||f.tipoComprobante==="") && <div style={{fontSize:11,color:"var(--orange)",marginTop:4}}>⚠️ Selecciona una opción para continuar</div>}
              {(f.tipoComprobante&&f.tipoComprobante!=="") && <div style={{fontSize:11,color:"var(--muted)",marginTop:4}}>
    ...
    <div className="modal-ov" onClick={onClose}>
      <div className="modal wide" style={{display:"flex",flexDirection:"column",height:"100%",maxHeight:"92vh"}} onClick={e => e.stopPropagation()}>
        ...
        <div className="mbody" style={{flex:1,overflowY:"auto",minHeight:0}}>
          ...
          {(Number(f.precioCliente) > 0 || Number(f.costoPagar) > 0 || Number(f.costoEstadias) > 0 || Number(f.gastosExtrasExt) > 0) && <div style={{ marginTop:6, marginBottom:10, padding:"12px 16px", background: utilidad >= 0 ? "#e8f8ee" : "#fdecea", borderRadius:10, border:`1px solid ${utilidad>=0?"var(--green)":"var(--red)"}` }}>
            <span style={{ fontSize: 11, color: "var(--muted)", fontWeight: 700 }}>📊 UTILIDAD ESTIMADA: </span>
    ...
    <div className="modal-ov" onClick={onClose}>
      <div className="modal wide" style={{display:"flex",flexDirection:"column",height:"100%",maxHeight:"92vh"}} onClick={e => e.stopPropagation()}>
        ...
        <div className="mbody" style={{flex:1,overflowY:"auto",minHeight:0}}>
          ...
          <div className="fg">
            <div className="field"><label>Días de Crédito</label><input type="number" value={f.diasCredito||""} onChange={chN("diasCredito")} min="0" placeholder="30"/></div>
            <div className="field"><label>Límite de Crédito ($)</label><input type="number" value={f.limiteCredito||""} onChange={chN("limiteCredito")} min="0" placeholder="50000"/></div>
            <div className="field"><label>Saldo Pendiente ($)</label><input type="number" value={f.saldoPendiente||""} onChange={chN("saldoPendiente")} min="0" placeholder=""/></div>
      ...
      <div className="modal wide" style={{display:"flex",flexDirection:"column",height:"100%",maxHeight:"92vh"}} onClick={e => e.stopPropagation()}>
        ...
        <div className="mbody" style={{flex:1,overflowY:"auto",minHeight:0}}>
          ...
          <div className="fg">
            ...
            <div className="field">
              <label>% Comisión por Viaje</label>
              <input value={editPct} onChange={e => setEditPct(Number(e.target.value) || 0)} type="number" min="0" max="100" step="0.1" placeholder="" />
          ...
          <div className="fg">
            ...
            <div className="field"><label>Bonos ($)</label><input value={bonos} onChange={e => setBonos(e.target.value)} type="number" min="0" placeholder="" /></div>
            <div className="field"><label>Estímulos de Productividad ($)</label><input value={estimulos} onChange={e => setEstimulos(e.target.value)} type="number" min="0" placeholder="" /></div>
          ...
          <div className="fg">
            <div className="field"><label>Total Deducciones ($)</label><input value={deducciones} onChange={e => setDeducciones(e.target.value)} type="number" min="0" placeholder="" /></div>
    ...
    <div className="modal-ov" onClick={onClose}>
      <div className="modal wide" style={{display:"flex",flexDirection:"column",height:"100%",maxHeight:"92vh"}} onClick={e => e.stopPropagation()}>
        ...
        <div className="mftr">
          ...
          <button className="btn btn-cyan" onClick={ok}>💾 Guardar Remisión</button>
        </div>
      </div>
    </div>
  );
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
            ...
            <div>
              ...
              <div className="fg">
                ...
                <div className="field"><label>Distancia (km)</label><input value={f.destinoDistancia} onChange={ch("destinoDistancia")} type="number" min="0" placeholder="" style={inp}/></div>
              </div>
            </div>
          </div>
    ...
    <div className="card">
      ...
      <div className="card-body">
        {fil.length === 0 ? <div className="empty"><div className="empty-icon">🗺️</div><p>Sin viajes</p></div> :
          <table>
            <thead><tr><th style={{width:32,textAlign:"center",color:"var(--muted)",fontSize:10}}>#</th><th>Tipo</th><th>Unidad/Empresa</th><th>Origen → Destino</th><th>Fecha</th><th>KM</th><th>Operador</th><th>Cliente</th><th>Tipo Comprobante</th><th>Evid.</th>{isAdmin && <th title="Ver utilidad del viaje">💰</th>}<th>Acciones</th></tr></thead>
            <tbody>{fil.map((t, idx) => {
              const u = t.tipo === "PROPIO" ? units.find(u => u.id === t.unidadId) : null;
                       ...
                       : t.remisionId
                         ...
                         : t.tipoComprobante === "factura"
                           ...
                           : t.tipoComprobante === "remision"
                             ...
                             : t.tipoComprobante === "ninguno"
                               ? <span style={{background:"var(--bg2)",color:"var(--muted)",padding:"2px 7px",borderRadius:20,fontWeight:700,fontSize:10}}>— Sin comprobante</span>
                ...
                <tbody>{maintsConProv.map(m => {
                  ...
                  return (
                    <tr key={m.id}>
                      ...
                      <td><div className="acts">{m.proveedorRefId && <button className="btn btn-cyan btn-xs" title={`Conciliar Refacciones: ${(proveedores||[]).find(p=>p.id===m.proveedorRefId)?.nombre||"—"}`} style={{marginBottom:m.proveedorId?2:0}} onClick={()=>setModalProvPago({item:{tipo:"mantenimiento_ref",id:m.id+"_ref",label:`Refac: ${m.descripcion||m.tipo||"—"}`,monto:Number(m.costoRef)||0,data:m},proveedor:(proveedores||[]).find(p=>p.id===m.proveedorRefId)})}>🔩 Ref</button>}{m.proveedorId && <button className="btn btn-cyan btn-xs" title={`Conciliar Taller: ${(proveedores||[]).find(p=>p.id===m.proveedorId)?.nombre||"—"}`} onClick={()=>setModalProvPago({item:{tipo:"mantenimiento_mo",id:m.id+"_mo",label:`Taller: ${m.descripcion||m.tipo||"—"}`,monto:Number(m.costoMO)||0,data:m},proveedor:(proveedores||[]).find(p=>p.id===m.proveedorId)})}>🔧 M.O.</button>}</div></td>
                    </tr>
                  );
                })}</tbody>
              </table>
                ...
                <tbody>{maintsConProv.map(m => {
                  ...
                  return (
                    <tr key={m.id}>
                      ...
                      <td><div className="acts">{m.proveedorRefId && <button className="btn btn-cyan btn-xs" title={`Conciliar Refacciones`} style={{marginBottom:m.proveedorId?2:0}} onClick={()=>setModalProvPago({item:{tipo:"mantenimiento_ref",id:m.id+"_ref",label:`Refac: ${m.descripcion||m.tipo||"—"}`,monto:Number(m.costoRef)||0,data:m},proveedor:(proveedores||[]).find(p=>p.id===m.proveedorRefId)})}>🔩 Ref</button>}{m.proveedorId && <button className="btn btn-cyan btn-xs" title={`Conciliar Taller`} onClick={()=>setModalProvPago({item:{tipo:"mantenimiento_mo",id:m.id+"_mo",label:`Taller: ${m.descripcion||m.tipo||"—"}`,monto:Number(m.costoMO)||0,data:m},proveedor:(proveedores||[]).find(p=>p.id===m.proveedorId)})}>🔧 M.O.</button>}</div></td>
                    </tr>
                  );
                })}</tbody>
              </table>
...
function ChartsPage({ units, maints, fuels, gastos, trips, facturas, clientes, drivers = [], proveedores = [], externos = [], nominasAdmin = [] }) {
  ...
  const meses12 = Array(12).fill(0).map((_,i)=>{
    ...
    return { fuel:mFuel, maint:mMaint, gast:mGast, extCosto:mExtCosto, ext:mExtN, ing:mIng, fact:mFact, costos:mFuel+mMaint+mGast+mExtCosto, viajes:mViaj, litros:mLit, mantN:mMantN };
  });
...
<div class="footer"><span>${co}</span><span>Generado: ${new Date().toLocaleDateString("es-MX",{year:"numeric",month:"long",day:"numeric"})}</span></div>
  ...
  return (
      ...
      {modal?.type === "trip" && <TripModal key={modal.data?.id || modal._ts || "new-trip"} trip={modal.data} units={units} drivers={drivers} clientes={clientes} rutasCatalogo={rutasCatalogo} tiposPersonalizados={tiposPersonalizados} onSaveRuta={r=>setRutasCatalogo(p=>[...p,r])} onSave={t => {
      ...
      {modal?.type === "factura" && <FacturaModal key={modal.data?.id || modal._ts || "new-fact"} factura={modal.data} clientes={clientes} branding={branding} viajes={trips} onSave={f => FacC.save(f)} onClose={() => setModal(null)} />}
